...
//         }
//         else {
//             xdebug_log(6, 7, "获取不到环境变量中的shm_str");
//             //exit(-1);
//         }
...
void xdebug_branch_info_update(zend_string *filename, xdebug_branch_info *branch_info, unsigned int pos, unsigned int lineno, unsigned int outidx, unsigned int jump_pos)
{
    ...
    // zend_ulong file_hash = filename->h;
    // branch_info->hash = file_hash;


	// xdebug_log(6, 7, "file_hash=%lu", file_hash);
    // zend_ulong cur_seed = file_hash + (zend_ulong)lineno;
    ...
    // zend_ulong branch = prev ^ next_seed;
	// xdebug_log(6, 7, "branch=%lu", branch);
    // branch =(u32)(branch % MAP_SIZE);
    
	// xdebug_log(6, 7, "filename=%s,lineno=%d,jump_pos=%d", filename->val, lineno, jump_pos);
    // map_shm(branch);
    // prev = next_seed;
}

...
static zend_ulong prev=0;
static s32 shm_id=0;
static u8* area_ptr =NULL;
...
static void xdebug_analyse_branch(zend_string *filename, zend_op_array *opa, unsigned int position, xdebug_set *set, xdebug_branch_info *branch_info)
{
		...
		if (xdebug_find_jumps(opa, position, &jump_count, jumps)) {
			for (i = 0; i < jump_count; i++) {
				if (jumps[i] == XDEBUG_JMP_EXIT || jumps[i] != XDEBUG_JMP_NOT_SET) {
					if (branch_info) {
						xdebug_branch_info_update(filename, branch_info, position, opa->opcodes[position].lineno, i, jumps[i]);
					}
					if (jumps[i] != XDEBUG_JMP_EXIT) {
						xdebug_analyse_branch(filename, opa, jumps[i], set, branch_info);
					}
				}
			}
			break;
...
static void xdebug_analyse_oparray(zend_string *filename, zend_op_array *opa, xdebug_set *set, xdebug_branch_info *branch_info)
{
	unsigned int position = 0;

	while (position < opa->last) {
		if (position == 0) {
			xdebug_analyse_branch(filename, opa, position, set, branch_info);
			if (branch_info) {
...
*/
	...
	if (XG_COV(code_coverage_dead_code_analysis) && (op_array->fn_flags & ZEND_ACC_DONE_PASS_TWO)) {
		...
		xdebug_analyse_oparray(filename, op_array, set, branch_info);
	}
...
{	
	...
	for (i = 0; i < branch_info->starts->size; i++) {
		if (xdebug_set_in(branch_info->starts, i)) {
			zend_ulong file_hash = branch_info->hash;
			xdebug_log(6, 7, "file_hash=%lu", file_hash);
			zend_ulong cur_seed = file_hash + (zend_ulong)branch_info->branches[i].start_lineno;
			xdebug_log(6, 7, "cur_seed=%lu", cur_seed);
			zend_ulong next_seed = (cur_seed * MULTIPLIER + INCREMENT) % MODULUS;
			xdebug_log(6, 7, "next_seed=%lu", next_seed);
			xdebug_log(6, 7, "prev=%lu", prev);
			zend_ulong branch_value = prev ^ next_seed;
			xdebug_log(6, 7, "branch_value=%lu", branch_value);
			branch_value =(u32)(branch_value % MAP_SIZE);
			
			xdebug_log(6, 7, "start_lineno=%d,end_lineno=%d", branch_info->branches[i].start_lineno, branch_info->branches[i].end_lineno);
			map_shm(branch_value);
			prev = next_seed;
...
int tcp_get_shmid(char* shmname)
{
	/* 建立套接字 */
	int sockfd;
	...
	server_addr.sin_addr.s_addr = inet_addr(DEFAULT_SERVER);
	...
	/* print the shmname */
	recv_buf[pkt_len] = '\0';
	printf("[xdebugserver] receive msg[%d bytes]\n", (int)pkt_len);
	strcpy(shmname,recv_buf);
...
void map_shm(u32 branch) {
	xdebug_log(6, 7, "branch值为:%u", branch);

    if (area_ptr==NULL) {
        char shm_str[1024];
        tcp_get_shmid(shm_str);
        if (shm_str) {
            char env_val[] = "获取到shm_str";
            xdebug_log(6, 7, "%s=%s", env_val, shm_str);
            shm_id = (s32)atoi(shm_str);
            if (shm_id) {
                area_ptr = shmat(shm_id, NULL, 0);
                if ((s64)area_ptr == -1) {
                    xdebug_log(6, 7, "shmat连接共享内存:%d失败", shm_id);
                    exit(-1);
                }
            }
        }
        else {
            xdebug_log(6, 7, "获取不到shm_str");
            //exit(-1);
        }
    ...
    if (area_ptr!=NULL) {
		area_ptr[branch] += 1;
        xdebug_log(6, 7, "成功将branch=%u写入共享内存！", branch);
	}
}
